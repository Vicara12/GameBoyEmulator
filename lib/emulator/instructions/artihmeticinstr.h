...
inline void handleFlags (Byte a, Byte b, State *state, FuncType f, bool added_carry)
{
  ...
  if (added_carry) {
    COND_SET_HALF_CARRY_FLAG(state, (f((a & 0x0F), 1) & 0xF0) != 0);
    COND_SET_HALF_CARRY_FLAG(state, (f((f(a, 1) & 0x0F), ((b-1) & 0x0F)) & 0xF0) != 0);
    COND_SET_CARRY_FLAG(state, (f(a, 1) & 0xFF00) != 0);
    COND_SET_CARRY_FLAG(state, (f(Byte(f(a, 1)), Byte(b-1)) & 0xFF00) != 0);
  } else {
...
{
  bool add_carry = ((state->F & CARRY_FLAG) != 0)*carry;
  r += Byte(add_carry);
